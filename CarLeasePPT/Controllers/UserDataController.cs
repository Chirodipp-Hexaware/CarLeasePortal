...
namespace CarLeasePPT.Controllers
{
    public class UserDataController : ApiController
    {
        ...
        private static UserListData DefaultData => new UserListData
        {
            draw = 0,
        ...
        [HttpPost]
        public async Task<UserListData> Post(DataTableAjaxPostModel model)
        {
            try
            {
                var request = HttpContext.Current.Request;
                if (!request.IsAuthenticated || !User.IsInRole("USER_MANAGEMENT")) return DefaultData;
                ...
                foreach (var v in returnData.data)
                    v.PersonStatus =
                        v.IsLocked
                            ? Resources.User_Common_Status_Locked_Message
                            : v.IsExpired
                                ? Resources.User_Common_Status_Expired_Message
                                : Resources.User_Common_Status_Active_Message;
        ...
        private static async Task<UserListData> ParseAndPullAsync(DataTableAjaxPostModel model)
        {
            // Grab the search and pagination values
            var searchBy = model.search?.value;
            var includeExpired = model.search?.show_excluded ?? false;
            ...
            var result = await PersonEngine.GetCollectionAsync(searchBy, take, skip, sortBy, sortDir, includeExpired);
